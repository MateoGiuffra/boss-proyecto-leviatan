...
# --- Hard breathing smooth fade (NEW) ---
@export var breathing_fade_time: float = 0.35
@export var breathing_idle_volume_db: float = -6.0
@export var breathing_hard_volume_db: float = 0.0
var _breathing_tween: Tween = null
var _is_hard_breathing_target: bool = false
...
# --- NEW: control de respiración sin cortes (crossfade) ---
func _play_sounds() -> void:
	var should_hard := has_low_oxygen()
	if should_hard != _is_hard_breathing_target:
		_is_hard_breathing_target = should_hard
		_set_hard_breathing(_is_hard_breathing_target)


func has_low_oxygen() -> bool:
	return oxygen <= (max_oxygen / 3.0)


func _reset_breathing_audio() -> void:
	# mata tween si había
	if _breathing_tween and _breathing_tween.is_running():
		_breathing_tween.kill()
	_breathing_tween = null

	# deja ambos en un estado conocido
	idle_sound.stop()
	hard_breathing_sound.stop()

	idle_sound.volume_db = breathing_idle_volume_db
	hard_breathing_sound.volume_db = -80.0

	_is_hard_breathing_target = false
	idle_sound.play()


func _set_hard_breathing(enable: bool) -> void:
	if _breathing_tween and _breathing_tween.is_running():
		_breathing_tween.kill()

	_breathing_tween = get_tree().create_tween()

	if enable:
		# Hard breathing entra suave
		if not hard_breathing_sound.playing:
			hard_breathing_sound.volume_db = -80.0
			hard_breathing_sound.play()

		if idle_sound.playing:
			_breathing_tween.tween_property(idle_sound, "volume_db", -80.0, breathing_fade_time)

		_breathing_tween.tween_property(hard_breathing_sound, "volume_db", breathing_hard_volume_db, breathing_fade_time)

		_breathing_tween.tween_callback(func():
			if idle_sound.playing and idle_sound.volume_db <= -60.0:
				idle_sound.stop()
		)
	else:
		# Volver a idle suave
		if not idle_sound.playing:
			idle_sound.volume_db = -80.0
			idle_sound.play()

		if hard_breathing_sound.playing:
			_breathing_tween.tween_property(hard_breathing_sound, "volume_db", -80.0, breathing_fade_time)

		_breathing_tween.tween_property(idle_sound, "volume_db", breathing_idle_volume_db, breathing_fade_time)

		_breathing_tween.tween_callback(func():
			if hard_breathing_sound.playing and hard_breathing_sound.volume_db <= -60.0:
				hard_breathing_sound.stop()
		)
...
func desactivate(hide_player = true):
	...
	# NEW: cortar tween para que no siga tocando volumen en background
	if _breathing_tween and _breathing_tween.is_running():
		_breathing_tween.kill()
	_breathing_tween = null
